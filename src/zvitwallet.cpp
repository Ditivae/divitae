...
CzDIVITWallet::CzDIVITWallet(std::string strWalletFile)
{
        ...
        if (walletdb.ReadZVITSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVITSeed_deprecated()) {
                    LogPrintf("%s: Updated zDIVIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zDIVIT seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zDIVIT wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDIVITWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
...
void CzDIVITWallet::Lock()
{
    seedMaster = 0;
}

void CzDIVITWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDIVITWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CzDIVITWallet::LoadMintPoolFromDB()
{
...
void CzDIVITWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDIVITWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDIVITWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzDIVITWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
...
void CzDIVITWallet::SeedToZPIV(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDIVITWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDIVITWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZVITCount(nCountLastUsed);
...
void CzDIVITWallet::GenerateDeterministicZPIV(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDIVITWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDIVITWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
