...
UniValue fnbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a DIVIT initiative\n"
            "  vote-alias         - Vote on a DIVIT initiative\n"
            "  vote               - Vote on a DIVIT initiative/budget\n"
            "  getvotes           - Show current prosperitynode budgets\n"
            "  getinfo            - Show current prosperitynode budgets\n"
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"Divitae-address\" monthy-payment\n"
            ...
            "5. \"Divitae-address\":   (string, required) DIVIT address to send payments to\n"
            "6. monthly-payment:  (numeric, required) Monthly payment amount\n"

            "\nResult:\n"
            "\"xxxx\"       (string) proposal fee hash (if successful) or error message (if failed)\n"
            "\nExamples:\n" +
            HelpExampleCli("preparebudget", "\"test-proposal\" \"https://forum.Divitae.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500") +
            HelpExampleRpc("preparebudget", "\"test-proposal\" \"https://forum.Divitae.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500"));
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"Divitae-address\" monthy-payment \"fee-tx\"\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("submitbudget", "\"test-proposal\" \"https://forum.Divitae.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500") +
            HelpExampleRpc("submitbudget", "\"test-proposal\" \"https://forum.Divitae.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500"));
    ...
    if (!prosperitynodeSync.IsBlockchainSynced()) {
        throw runtime_error("Must wait for client to sync with prosperitynode network. Try again in a minute or so.");
    }
    ...
    budget.mapSeenProsperitynodeBudgetProposals.insert(make_pair(budgetProposalBroadcast.GetHash(), budgetProposalBroadcast));
...
UniValue fnbudgetvote(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() == 3 && (strCommand != "local" && strCommand != "many")) || (params.size() == 4 && strCommand != "alias") ||
        ...
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"mode\"      (string, required) The voting mode. 'local' for voting directly from a prosperitynode, 'many' for voting with a MN controller and casting the same vote for each MN, 'alias' for voting with a MN controller and casting a vote for a single MN\n"
    ...
    if (strCommand == "local") {
        ...
        while (true) {
            if (!obfuScationSigner.SetKey(strFundamentalNodePrivKey, errorMessage, keyProsperitynode, pubKeyProsperitynode)) {
                ...
                statusObj.push_back(Pair("error", "Prosperitynode signing error, could not set key correctly: " + errorMessage));
            ...
            if (pmn == NULL) {
                ...
                statusObj.push_back(Pair("node", "local"));
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("error", "Failure to find prosperitynode in list : " + activeProsperitynode.vin.ToString()));
                resultsObj.push_back(statusObj);
                break;
            }

            CBudgetVote vote(activeProsperitynode.vin, hash, nVote);
            if (!vote.Sign(keyProsperitynode, pubKeyProsperitynode)) {
                failed++;
                statusObj.push_back(Pair("node", "local"));
                statusObj.push_back(Pair("result", "failed"));
            ...
            if (budget.UpdateProposal(vote, NULL, strError)) {
                success++;
                budget.mapSeenProsperitynodeBudgetVotes.insert(make_pair(vote.GetHash(), vote));
                vote.Relay();
                statusObj.push_back(Pair("node", "local"));
    ...
    if (strCommand == "many") {
        BOOST_FOREACH (CProsperitynodeConfig::CProsperitynodeEntry mne, prosperitynodeConfig.getEntries()) {
            ...
            if (!obfuScationSigner.SetKey(mne.getPrivKey(), errorMessage, keyProsperitynode, pubKeyProsperitynode)) {
                failed++;
            ...
            CProsperitynode* pmn = mnodeman.Find(pubKeyProsperitynode);
            if (pmn == NULL) {
                failed++;
                statusObj.push_back(Pair("node", mne.getAlias()));
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("error", "Can't find prosperitynode by pubkey"));
                resultsObj.push_back(statusObj);
                continue;
            }
...
UniValue getbudgetvotes(const UniValue& params, bool fHelp)
{
    if (params.size() != 1)
        throw runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"mnId\": \"xxxx\",        (string) Hash of the prosperitynode's collateral transaction\n"
...
UniValue getbudgetprojection(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) DIVIT address of payment\n"
...
UniValue getbudgetinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nShow current prosperitynode budgets\n"
...
UniValue fnbudgetrawvote(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "fnbudgetrawvote \"prosperitynode-tx-hash\" prosperitynode-tx-index \"proposal-hash\" yes|no time \"vote-sig\"\n"
            "\nCompile and relay a proposal vote with provided external signature instead of signing vote internally\n"

            "\nArguments:\n"
            "1. \"prosperitynode-tx-hash\"  (string, required) Transaction hash for the prosperitynode\n"
            "2. prosperitynode-tx-index   (numeric, required) Output index for the prosperitynode\n"
    ...
    if (pmn == NULL) {
        return "Failure to find prosperitynode in list : " + vin.ToString();
    }
...
UniValue fnfinalbudget(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "vote-many") {
        ...
        BOOST_FOREACH (CProsperitynodeConfig::CProsperitynodeEntry mne, prosperitynodeConfig.getEntries()) {
            ...
            if (!obfuScationSigner.SetKey(mne.getPrivKey(), errorMessage, keyProsperitynode, pubKeyProsperitynode)) {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Prosperitynode signing error, could not set key correctly: " + errorMessage));
            ...
            if (pmn == NULL) {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Can't find prosperitynode by pubkey"));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }
    ...
    if (strCommand == "vote") {
        ...
        CProsperitynode* pmn = mnodeman.Find(activeProsperitynode.vin);
        if (pmn == NULL) {
            return "Failure to find prosperitynode in list : " + activeProsperitynode.vin.ToString();
        }

        CFinalizedBudgetVote vote(activeProsperitynode.vin, hash);
        if (!vote.Sign(keyProsperitynode, pubKeyProsperitynode)) {
