...
CObfuScationRelay::CObfuScationRelay(CTxIn& vinProsperitynodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinProsperitynode = vinProsperitynodeIn;
    vchSig = vchSigIn;
...
std::string CObfuScationRelay::ToString()
{
    ...
    info << "vin: " << vinProsperitynode.ToString() << " nBlockHeight: " << (int)nBlockHeight << " nRelayType: " << (int)nRelayType << " in " << in.ToString() << " out " << out.ToString();
...
{
    CProsperitynode* pmn = mnodeman.GetProsperitynodeByRank(nRank, nBlockHeight, ActiveProtocol());
