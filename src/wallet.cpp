...
/**
 * Fees smaller than this (in uDIVIT) are considered zero fee (for transaction creation)
...
bool CWallet::GetProsperitynodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetProsperitynodeVinAndKeys -- Could not locate any valid prosperitynode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetProsperitynodeVinAndKeys -- Could not locate specified prosperitynode vin\n");
    return false;
}
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add prosperitynode & masternode collaterals which are handled likc locked coins
...
CAmount CWalletTx::GetLockedWatchOnlyCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add prosperitynode collaterals which are handled likc locked coins
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-zvitstake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zDIVIT set once per update interval
        bool fUpdate = false;
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 DIVIT
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 DIVIT over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        // prosperitynode-like input should not be selected by AvailableCoins now anyway
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 DIVIT+1 ( bit on if present )
            // bit 1 - 1000 DIVIT+1
            // bit 2 - 100 DIVIT+1
            // bit 3 - 10 DIVIT+1
            // bit 4 - 1 DIVIT+1
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (/*fFundamentalNode && */out.tx->vout[out.i].nValue == FN_MAGIC_AMOUNT) continue; //prosperitynode input
        if (fMasterNode && out.tx->vout[out.i].nValue == MASTERNODEAMOUNT * COIN) continue; //prosperitynode input
            ...
            while (true) {
                ...
                if (coinControl && !coinControl->fSplitBlock) {
                    ...
                    if(IsFundamentalNodePayment){
                        ...
                        if(nTotalValue < PROSPERITYNODE_AMOUNT){
                            return false;
                        }
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 DIVIT.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 DIVIT.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-Divitae-address
                    CScript scriptChange;
...
void CWallet::AutoCombineDust()
{
    ...
    for (map<CBitcoinAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
        ...
        //find prosperitynode rewards that need to be combined
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        bool sendMSonFNReward = fMultiSendProsperitynodeReward && outpoint.IsProsperitynodeReward(out.tx);
        ...
        if (!walletdb.WriteMSettings(fMultiSendStake, fMultiSendProsperitynodeReward,fMultiSendMasternodeReward, nLastMultiSendHeight))
        ...
        if ((stakeSent && mnSent) || (stakeSent && !fMultiSendProsperitynodeReward) || (mnSent && !fMultiSendStake))
            return true;
    }
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to DIVIT
    ...
    // Sign if these are Divitae outputs - NOTE that zDIVIT outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zDIVIT privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        //Add the coin spend into a DIVIT transaction
        ...
        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            //Tried to spend an already spent zDIVIT
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the the smallest available denomination (= 1) of zDIVIT"), nStatus);
        return false;
    }
    ...
    if (vSelectedMints.empty()) {
        ...
        // Select the zDIVIT mints to use in this spend
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                ...
                receipt.SetStatus(_("Version 1 zDIVIT require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to Divitae address to the transaction (the actual primary spend taking place)
...
void CWallet::ZVitBackupWallet()
{
    ...
    if(!GetArg("-zDivitaebackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zDivitaebackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    receipt.SetStatus("Spend Successful", ZVIT_SPEND_OKAY);  // When we reach this point spending zDIVIT was successful
