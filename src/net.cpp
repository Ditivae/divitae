...
{
    if (pszDest == NULL) {
        // we clean prosperitynode connections in CProsperitynodeMan::ProcessProsperitynodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActiveProsperitynode::ManageStatus()
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (DIVIT users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "DIVIT " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. DIVIT Core is probably already running."), addrBind.ToString());
        else
