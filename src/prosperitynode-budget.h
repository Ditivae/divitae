...
#ifndef PROSPERITYNODE_BUDGET_H
#define PROSPERITYNODE_BUDGET_H
...
//
// CBudgetVote - Allow a prosperitynode node to vote and broadcast throughout the network
//
...
//
// CFinalizedBudgetVote - Allow a prosperitynode node to vote and broadcast throughout the network
//
...
public:
    ...
    std::map<uint256, CBudgetProposalBroadcast> mapSeenProsperitynodeBudgetProposals;
    std::map<uint256, CBudgetVote> mapSeenProsperitynodeBudgetVotes;
    std::map<uint256, CBudgetVote> mapOrphanProsperitynodeBudgetVotes;
    ...
    void ClearSeen()
    {
        mapSeenProsperitynodeBudgetProposals.clear();
        mapSeenProsperitynodeBudgetVotes.clear();
    ...
    void Clear()
    {
        ...
        mapSeenFinalizedBudgets.clear();
        mapSeenFinalizedBudgetVotes.clear();
        mapOrphanProsperitynodeBudgetVotes.clear();
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion)
    {
        READWRITE(mapSeenProsperitynodeBudgetProposals);
        READWRITE(mapSeenProsperitynodeBudgetVotes);
        READWRITE(mapSeenFinalizedBudgets);
        READWRITE(mapSeenFinalizedBudgetVotes);
        READWRITE(mapOrphanProsperitynodeBudgetVotes);
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (prosperitynode only)
...
public:
    ...
    void AutoCheck();
    //total Divitae paid out by this budget
    CAmount GetTotalPayout();
    //vote on this finalized budget as a prosperitynode
    void SubmitVote();
...
//
// Budget Proposal : Contains the prosperitynode votes for each budget
//
