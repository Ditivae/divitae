...
bool COutPoint::IsProsperitynodeReward(const CTransaction* tx) const
{
    if(!tx->IsCoinStake())
        return false;
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // DIVIT: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
