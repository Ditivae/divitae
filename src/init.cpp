...
#include "config/Divitae-config.h"
#endif
...
CzDIVITWallet* zwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "Divitae.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "Divitaed.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the DIVIT and zDIVIT money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 9765, 51474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in DIVIT/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in DIVIT/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, Divitae, (obfuscation, swiftx, prosperitynode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in DIVIT/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DIVIT specific functionality (Prosperitynodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-zvitstake=<n>", strprintf(_("Enable or disable staking functionality for zDIVIT inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageGroup(_("Prosperitynode options:"));
    strUsage += HelpMessageOpt("-prosperitynode=<n>", strprintf(_("Enable the client to act as a prosperitynode (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-fnconf=<file>", strprintf(_("Specify prosperitynode configuration file (default: %s)"), "prosperitynode.conf"));
    strUsage += HelpMessageOpt("-fnconflock=<n>", strprintf(_("Lock prosperitynodes from prosperitynode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-prosperitynodeprivkey=<n>", _("Set the prosperitynode private key"));
    strUsage += HelpMessageOpt("-prosperitynodeaddr=<n>", strprintf(_("Set external address:port to get to this prosperitynode (example: %s)"), "128.127.106.235:9765"));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "127.0.0.2:9765"));
    strUsage += HelpMessageOpt("-budgetvotemode=<mode>", _("Change automatic finalized budget voting behavior. mode=auto: Vote for only exact finalized budget match to my generated budget. (string, default: auto)"));
    ...
    strUsage += HelpMessageOpt("-backupzVit=<n>", strprintf(_("Enable automatic wallet backups triggered after each zDIVIT minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zDivitaebackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zDIVIT backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeDivitaeamount=<n>     " + strprintf(_("Keep N PIV anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 11473, 51475));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The DIVIT Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_(" ")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that DIVIT is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DIVIT Core is shutting down."));
...
#endif
    // Make sure only a single DIVIT process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DIVIT Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //DIVIT specific: zerocoin and spork DB's
                ...
                // DIVIT: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DIVIT Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart DIVIT Core to complete") << "\n";
        ...
        zwalletMain = new CzDIVITWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zDIVITWallet
        uiInterface.InitMessage(_("Syncing zDIVIT wallet..."));
    ...
    uiInterface.InitMessage(_("Loading prosperitynode cache..."));

    CProsperitynodeDB mndb;
    CProsperitynodeDB::ReadResult readResult = mndb.Read(mnodeman);
    if (readResult == CProsperitynodeDB::FileError)
        LogPrintf("Missing prosperitynode cache file - fncache.dat, will try to recreate\n");
...
/*
    ...
    if (readResult == CMasternodeDB::FileError)
        LogPrintf("Missing prosperitynode cache file - mncache.dat, will try to recreate\n");
...
*/
    ...
    uiInterface.InitMessage(_("Loading prosperitynode payment cache..."));

    CProsperitynodePaymentDB mnpayments;
    CProsperitynodePaymentDB::ReadResult readResult3 = mnpayments.Read(prosperitynodePayments);

    if (readResult3 == CProsperitynodePaymentDB::FileError)
        LogPrintf("Missing prosperitynode payment cache - mnpayments.dat, will try to recreate\n");
    else if (readResult3 != CProsperitynodePaymentDB::Ok) {
        LogPrintf("Error reading mnpayments.dat: ");
        if (readResult3 == CProsperitynodePaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }

    fFundamentalNode = GetBoolArg("-prosperitynode", false);

    if ((fFundamentalNode || prosperitynodeConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling Prosperitynode support requires turning on transaction indexing."
                         "Please add txindex=1 to your configuration and start with -reindex");
    }

    if (fFundamentalNode) {
        LogPrintf("IS FUNDAMNENTAL NODE\n");
        strFundamentalNodeAddr = GetArg("-prosperitynodeaddr", "");
        ...
        if (!strFundamentalNodeAddr.empty()) {
            ...
            if (!addrTest.IsValid()) {
                return InitError("Invalid -prosperitynodeaddr address: " + strFundamentalNodeAddr);
            }
        }

        strFundamentalNodePrivKey = GetArg("-prosperitynodeprivkey", "");
        if (!strFundamentalNodePrivKey.empty()) {
            ...
            if (!obfuScationSigner.SetKey(strFundamentalNodePrivKey, errorMessage, key, pubkey)) {
                return InitError(_("Invalid prosperitynodeprivkey. Please see documenation."));
            }

            activeProsperitynode.pubKeyProsperitynode = pubkey;

        } else {
            return InitError(_("You must specify a prosperitynodeprivkey in the configuration. Please see documentation for help."));
        }
    }
    ...
    uiInterface.InitMessage(_("Loading masternode cache..."));
        ...
        if(fMasterNode) {
            ...
            if(!strMasterNodeAddr.empty()){
                CService addrTest = CService(strMasterNodeAddr);
                if (!addrTest.IsValid()) {
                    return InitError("Invalid -prosperitynodeaddr address: " + strMasterNodeAddr);
                }
            }
    ...
    if (GetBoolArg("-fnconflock", true) && pwalletMain) {
        ...
        LogPrintf("Locking Prosperitynodes:\n");
        uint256 mnTxHash;
...
//    }
//
//    nAnonymizeDivitaeAmount = GetArg("-anonymizeDivitaeamount", 0);
//    if (nAnonymizeDivitaeAmount > 999999) nAnonymizeDivitaeAmount = 999999;
//    if (nAnonymizeDivitaeAmount < 2) nAnonymizeDivitaeAmount = 2;

    fEnableSwiftTX = GetBoolArg("-enableswifttx", fEnableSwiftTX);
    nSwiftTXDepth = GetArg("-swifttxdepth", nSwiftTXDepth);
    nSwiftTXDepth = std::min(std::max(nSwiftTXDepth, 0), 60);

    //lite mode disables all Prosperitynode and Obfuscation related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fFundamentalNode && fLiteMode) {
        return InitError("You can not start a prosperitynode in litemode");
    }

    LogPrintf("fLiteMode %d\n", fLiteMode);
    LogPrintf("nSwiftTXDepth %d\n", nSwiftTXDepth);
    LogPrintf("Anonymize DIVIT Amount %d\n", nAnonymizeDivitaeAmount);
