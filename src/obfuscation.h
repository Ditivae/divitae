...
#define PROSPERITYNODE_ACCEPTED 1
#define PROSPERITYNODE_REJECTED 0
#define PROSPERITYNODE_RESET -1
...
extern CActiveProsperitynode activeProsperitynode;
...
public:
     ...
     *     1) we have an active Prosperitynode,
     *     2) we have a valid Prosperitynode private key,
    ...
    /// Check if we have a valid Prosperitynode address
    bool CheckSignature();
};
...
private:
    ...
    std::vector<CObfuScationEntry> entries; // Prosperitynode/clients entries
    ...
    bool sessionFoundProsperitynode; //If we've found a compatible Prosperitynode
...
public:
    ...
    CProsperitynode* pSubmittedToProsperitynode;
    ...
    void UpdateState(unsigned int newState)
    {
        if (fFundamentalNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            // LogPrint("obfuscation", "CObfuscationPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Prosperitynode. \n");
            return;
        }
    ...
    /// As a client, send a transaction to a Prosperitynode to start the denomination process
    void SendObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount);
    /// Get Prosperitynode updates about the progress of Obfuscation
    ...
    /// Get the denominations for a specific amount of Divitae.
