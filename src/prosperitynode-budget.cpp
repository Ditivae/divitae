...
void CBudgetManager::CheckOrphanVotes()
{
    ...
    std::string strError = "";
    std::map<uint256, CBudgetVote>::iterator it1 = mapOrphanProsperitynodeBudgetVotes.begin();
    while (it1 != mapOrphanProsperitynodeBudgetVotes.end()) {
        if (budget.UpdateProposal(((*it1).second), NULL, strError)) {
            ...
            mapOrphanProsperitynodeBudgetVotes.erase(it1++);
        } else {
            ++it1;
        }
    }
...
CBudgetDB::CBudgetDB()
{
    ...
    strMagicMessage = "ProsperitynodeBudget";
}
...
bool CBudgetManager::IsBudgetPaymentBlock(int nBlockHeight)
{
    ...
    LogPrint("fnbudget","CBudgetManager::IsBudgetPaymentBlock() - nHighestCount: %lli, 5%% of Prosperitynodes: %lli. Number of budgets: %lli\n",
              nHighestCount, nFivePercent, mapFinalizedBudgets.size());

    // If budget doesn't have 5% of the network votes, then we should pay a prosperitynode instead
...
TrxValidationStatus CBudgetManager::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    LogPrint("fnbudget","CBudgetManager::IsTransactionValid() - nHighestCount: %lli, 5%% of Prosperitynodes: %lli mapFinalizedBudgets.size(): %ld\n",
              nHighestCount, nFivePercent, mapFinalizedBudgets.size());
    /*
        If budget doesn't have 5% of the network votes, then we should pay a prosperitynode instead
    */
...
void CBudgetManager::NewBlock()
{
    ...
    if (prosperitynodeSync.RequestedProsperitynodeAssets <= PROSPERITYNODE_SYNC_BUDGET) return;
    ...
    if (prosperitynodeSync.IsSynced()) {
...
void CBudgetManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "fnvs") { //Prosperitynode vote sync
        uint256 nProp;
        vRecv >> nProp;
        ...
        Sync(pfrom, nProp);
        LogPrint("mnbudget", "fnvs - Sent Prosperitynode votes to peer %i\n", pfrom->GetId());
    }

    if (strCommand == "fprop") { //Prosperitynode Proposal
        CBudgetProposalBroadcast budgetProposalBroadcast;
        vRecv >> budgetProposalBroadcast;

        if (mapSeenProsperitynodeBudgetProposals.count(budgetProposalBroadcast.GetHash())) {
            prosperitynodeSync.AddedBudgetItem(budgetProposalBroadcast.GetHash());
            return;
        }
        ...
        mapSeenProsperitynodeBudgetProposals.insert(make_pair(budgetProposalBroadcast.GetHash(), budgetProposalBroadcast));
    ...
    if (strCommand == "fvote") { //Prosperitynode Vote
        CBudgetVote vote;
        vRecv >> vote;
        vote.fValid = true;

        if (mapSeenProsperitynodeBudgetVotes.count(vote.GetHash())) {
            prosperitynodeSync.AddedBudgetItem(vote.GetHash());
            return;
        }

        CProsperitynode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("fnbudget","fvote - unknown prosperitynode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }


        mapSeenProsperitynodeBudgetVotes.insert(make_pair(vote.GetHash(), vote));
    ...
    if (strCommand == "fbs") { //Finalized Budget Suggestion
        ...
        if (mapSeenFinalizedBudgets.count(finalizedBudgetBroadcast.GetHash())) {
            prosperitynodeSync.AddedBudgetItem(finalizedBudgetBroadcast.GetHash());
            return;
        }
    ...
    if (strCommand == "fbvote") { //Finalized Budget Vote
        ...
        CProsperitynode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("mnbudget", "fbvote - unknown prosperitynode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }
...
void CBudgetManager::ResetSync()
{
    ...
    std::map<uint256, CBudgetProposalBroadcast>::iterator it1 = mapSeenProsperitynodeBudgetProposals.begin();
    while (it1 != mapSeenProsperitynodeBudgetProposals.end()) {
...
void CBudgetManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    */
    ...
    pfrom->PushMessage("ssc", PROSPERITYNODE_SYNC_BUDGET_PROP, nInvCount);
    ...
    pfrom->PushMessage("ssc", PROSPERITYNODE_SYNC_BUDGET_FIN, nInvCount);
    LogPrint("mnbudget", "CBudgetManager::Sync - sent %d items\n", nInvCount);
}
...
// If prosperitynode voted for a proposal, but is now invalid -- remove the vote
...
bool CBudgetVote::Sign(CKey& keyProsperitynode, CPubKey& pubKeyProsperitynode)
{
    // Choose coins to use
...
bool CBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        if (fDebug){
            LogPrint("fnbudget","CBudgetVote::SignatureValid() - Unknown Prosperitynode - %s\n", vin.prevout.hash.ToString());
        }
        return false;
    }
...
//evaluate if we should vote for this. Prosperitynode only
...
void CFinalizedBudget::SubmitVote()
{
    ...
    CFinalizedBudgetVote vote(activeProsperitynode.vin, GetHash());
...
bool CFinalizedBudgetVote::Sign(CKey& keyProsperitynode, CPubKey& pubKeyProsperitynode)
{
    // Choose coins to use
    CPubKey pubKeyCollateralAddress;
    CKey keyCollateralAddress;
...
bool CFinalizedBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        LogPrint("fnbudget","CFinalizedBudgetVote::SignatureValid() - Unknown Prosperitynode %s\n", strMessage);
        return false;
    }
...
std::string CBudgetManager::ToString() const
{
    ...
    info << "Proposals: " << (int)mapProposals.size() << ", Budgets: " << (int)mapFinalizedBudgets.size() << ", Seen Budgets: " << (int)mapSeenProsperitynodeBudgetProposals.size() << ", Seen Budget Votes: " << (int)mapSeenProsperitynodeBudgetVotes.size() << ", Seen Final Budgets: " << (int)mapSeenFinalizedBudgets.size() << ", Seen Final Budget Votes: " << (int)mapSeenFinalizedBudgetVotes.size();
