...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DIVIT at the same time
    ONLY_10000 = 5,                        // find prosperitynode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDIVIT send
...
public:
    ...
    */
    ...
    CzDIVITWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDIVITTracker> zvitTracker;
    ...
    bool fMultiSendProsperitynodeReward;
    ...
    void SetNull()
    {
        ...
        fMultiSendProsperitynodeReward = false;
    ...
    void setZWallet(CzDIVITWallet* zwallet)
    {
        zwalletMain = zwallet;
        zvitTracker = std::unique_ptr<CzDIVITTracker>(new CzDIVITTracker(strWalletFile));
    }

    CzDIVITWallet* getZWallet() { return zwalletMain; }
    ...
    {
        return fMultiSendProsperitynodeReward || fMultiSendStake;
    }
    ...
    /// Get 1000DASH output and keys which can be used for the Prosperitynode
    bool GetProsperitynodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash = "", std::string strOutputIndex = "");
...
public:
    ...
    int Priority() const
    {
        BOOST_FOREACH (CAmount d, obfuScationDenominations)
            if (tx->vout[i].nValue == d) return 2500;
