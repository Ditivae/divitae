...
//step 2.) Top SWIFTTX_SIGNATURES_TOTAL prosperitynodes, open connect to top 1 prosperitynode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 prosperitynode, waits for SWIFTTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
...
void ProcessMessageSwiftTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    } else if (strCommand == "txlvote") // SwiftX Lock Consensus Votes
    {
        ...
        if (ProcessConsensusVote(pfrom, ctx)) {
            //Spam/Dos protection
            /*
                Prosperitynodes will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                if (!mapUnknownVotes.count(ctx.vinProsperitynode.prevout.hash)) {
                    mapUnknownVotes[ctx.vinProsperitynode.prevout.hash] = GetTime() + (60 * 10);
                }

                if (mapUnknownVotes[ctx.vinProsperitynode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinProsperitynode.prevout.hash] - GetAverageVoteTime() > 60 * 10) {
                    LogPrintf("ProcessMessageSwiftTX::ix - prosperitynode is spamming transaction votes: %s %s\n",
                        ctx.vinProsperitynode.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which prosperitynodes
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetProsperitynodeRank(activeProsperitynode.vin, nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    if (n == -1) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Unknown Prosperitynode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Prosperitynode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinProsperitynode = activeProsperitynode.vin;
...
{
    int n = mnodeman.GetProsperitynodeRank(ctx.vinProsperitynode, ctx.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    CProsperitynode* pmn = mnodeman.Find(ctx.vinProsperitynode);
    if (pmn != NULL)
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Prosperitynode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Unknown Prosperitynode\n");
        mnodeman.AskForMN(pnode, ctx.vinProsperitynode);
        return false;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Prosperitynode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("SwiftX::ProcessConsensusVote - Signature invalid\n");
        // don't ban, it could just be a non-synced prosperitynode
        mnodeman.AskForMN(pnode, ctx.vinProsperitynode);
        return false;
    }
...
{
    return vinProsperitynode.prevout.hash + vinProsperitynode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("SwiftX::CConsensusVote::SignatureValid() - Unknown Prosperitynode\n");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pmn->pubKeyProsperitynode, vchFundamentalNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    if (!obfuScationSigner.SetKey(strFundamentalNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid prosperitynodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetProsperitynodeRank(vote.vinProsperitynode, vote.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

        if (n == -1) {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Prosperitynode\n");
            return false;
        }

        if (n > SWIFTTX_SIGNATURES_TOTAL) {
            LogPrintf("CTransactionLock::SignaturesValid() - Prosperitynode not in the top %s\n", SWIFTTX_SIGNATURES_TOTAL);
            return false;
        }
