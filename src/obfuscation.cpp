...
// A helper object for signing messages from Prosperitynodes
...
// Keep track of the used Prosperitynodes
std::vector<CTxIn> vecProsperitynodesUsed;
// Keep track of the scanning errors I've seen
map<uint256, CObfuscationBroadcastTx> mapObfuscationBroadcastTxes;
// Keep track of the active Prosperitynode
CActiveProsperitynode activeProsperitynode;

/* *** BEGIN OBFUSCATION MAGIC - DIVIT **********
...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        if (pfrom->nVersion < ActiveProtocol()) {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PROSPERITYNODE_REJECTED, errorID);
        ...
        if (!fFundamentalNode) {
            ...
            LogPrintf("dsa -- not a Prosperitynode! \n");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PROSPERITYNODE_REJECTED, errorID);
        ...
        if (!IsCompatibleWithSession(nDenom, txCollateral, errorID)) {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PROSPERITYNODE_ACCEPTED, errorID);
            return;
        }
        ...
        CProsperitynode* pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        // if the queue is ready, submit if we can
        if (dsq.ready) {
            if (!pSubmittedToProsperitynode) return;
            if ((CNetAddr)pSubmittedToProsperitynode->addr != (CNetAddr)addr) {
                LogPrintf("dsq - message doesn't match current Prosperitynode - %s != %s\n", pSubmittedToProsperitynode->addr.ToString(), addr.ToString());
                return;
            }
            ...
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("obfuscation", "dsq -- Prosperitynode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
        ...
        if (!fFundamentalNode) {
            LogPrintf("dsi -- not a Prosperitynode! \n");
            errorID = ERR_NOT_A_MN;
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PROSPERITYNODE_REJECTED, errorID);
        ...
        if (AddEntry(in, nAmount, txCollateral, out, errorID)) {
            ...
            RelayStatus(sessionID, GetState(), GetEntriesCount(), PROSPERITYNODE_RESET);
        } else {
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PROSPERITYNODE_REJECTED, errorID);
        }
        ...
        if (!pSubmittedToProsperitynode) return;
        if ((CNetAddr)pSubmittedToProsperitynode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("dssu - message doesn't match current Prosperitynode - %s != %s\n", pSubmittedToProsperitynode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (success) {
            ...
            RelayStatus(obfuScationPool.sessionID, obfuScationPool.GetState(), obfuScationPool.GetEntriesCount(), PROSPERITYNODE_RESET);
        }
        ...
        if (!pSubmittedToProsperitynode) return;
        if ((CNetAddr)pSubmittedToProsperitynode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("dsc - message doesn't match current Prosperitynode - %s != %s\n", pSubmittedToProsperitynode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
void CObfuscationPool::SetNull()
{
    ...
    sessionFoundProsperitynode = false;
...
std::string CObfuscationPool::GetStatus()
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSuccess < minBlockSpacing || !prosperitynodeSync.IsBlockchainSynced()) {
    ...
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            ...
            if (showingObfuScationMessage % 70 <= 40)
                return strprintf(_("Submitted following entries to prosperitynode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                ...
                suffix = "...";
            return strprintf(_("Submitted to prosperitynode, waiting for more entries ( %u / %d ) %s"), entriesCount, GetMaxPoolTransactions(), suffix);
        }
    ...
    case POOL_STATUS_QUEUE:
        if (showingObfuScationMessage % 70 <= 30)
            ...
            suffix = "...";
        return strprintf(_("Submitted to prosperitynode, waiting in queue %s"), suffix);
        ;
    default:
...
//
// Check the Obfuscation progress and send client updates if a Prosperitynode
//
void CObfuscationPool::Check()
{
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        ...
        UnlockCoins();
        SetNull();
        if (fFundamentalNode) RelayStatus(sessionID, GetState(), GetEntriesCount(), PROSPERITYNODE_RESET);
    }
}

void CObfuscationPool::CheckFinalTransaction()
{
    if (!fFundamentalNode) return; // check and relay final tx only on prosperitynode
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        if (!obfuScationSigner.SetKey(strFundamentalNodePrivKey, strError, key2, pubkey2)) {
            LogPrintf("CObfuscationPool::Check() - ERROR: Invalid Prosperitynodeprivkey: '%s'\n", strError);
            return;
        }
        ...
        if (!mapObfuscationBroadcastTxes.count(txNew.GetHash())) {
            ...
            dstx.tx = txNew;
            dstx.vin = activeProsperitynode.vin;
            dstx.vchSig = vchSig;
...
//
// How does this work? Messages to Prosperitynodes come in via "dsi", these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Prosperitynode
    ...
    if (fFundamentalNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat DIVIT and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 DIVIT per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::CheckForCompleteQueue()
{
    ...
    if (state == POOL_STATUS_QUEUE && sessionUsers == GetMaxPoolTransactions()) {
        ...
        dsq.vin = activeProsperitynode.vin;
        dsq.time = GetTime();
        dsq.ready = true;
        dsq.Sign();
        dsq.Relay();
...
//
// Execute a Obfuscation denomination via a Prosperitynode.
// This is only ran from clients
//
void CObfuscationPool::SendObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount)
{
    if (fFundamentalNode) {
        LogPrintf("CObfuscationPool::SendObfuscationDenominate() - Obfuscation from a Prosperitynode is not supported currently.\n");
        return;
    }
    ...
    // we should already be connected to a Prosperitynode
    if (!sessionFoundProsperitynode) {
        LogPrintf("CObfuscationPool::SendObfuscationDenominate() - No Prosperitynode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
...
// Incoming message from Prosperitynode updating the progress of Obfuscation
...
bool CObfuscationPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    ...
    if (errorID != MSG_NOERR) strAutoDenomResult = _("Prosperitynode:") + " " + GetMessageByID(errorID);
    ...
    if (newAccepted != -1) {
        ...
        if (newAccepted == 1 && newSessionID != 0) {
            ...
            sessionFoundProsperitynode = true;
        }
    }
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            ...
            sessionFoundProsperitynode = true;
            //wait for other users. Prosperitynode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundProsperitynode) {
            LogPrintf("CObfuscationPool::StatusUpdate - entry not accepted by Prosperitynode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
            DoAutomaticDenominating(); //try another Prosperitynode
        }
        if (sessionFoundProsperitynode) return true;
    }
...
//
// After we receive the finalized transaction from the Prosperitynode, we must
...
bool CObfuscationPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    ...
    // push all of our signatures to the Prosperitynode
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (!prosperitynodeSync.IsBlockchainSynced()) {
    ...
    if (mnodeman.size() == 0) {
        LogPrint("obfuscation", "CObfuscationPool::DoAutomaticDenominating - No Prosperitynodes detected\n");
        strAutoDenomResult = _("No Prosperitynodes detected.");
        return false;
    }
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeDivitaeAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    // initial phase, find a Prosperitynode
    if (!sessionFoundProsperitynode) {
        ...
        //if we've used 90% of the Prosperitynode list then drop all the oldest first
        int nThreshold = (int)(mnodeman.CountEnabled(ActiveProtocol()) * 0.9);
        LogPrint("obfuscation", "Checking vecProsperitynodesUsed size %d threshold %d\n", (int)vecProsperitynodesUsed.size(), nThreshold);
        while ((int)vecProsperitynodesUsed.size() > nThreshold) {
            vecProsperitynodesUsed.erase(vecProsperitynodesUsed.begin());
            LogPrint("obfuscation", "  vecProsperitynodesUsed size %d threshold %d\n", (int)vecProsperitynodesUsed.size(), nThreshold);
        }
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                bool fUsed = false;
                //don't reuse Prosperitynodes
                BOOST_FOREACH (CTxIn usedVin, vecProsperitynodesUsed) {
                    if (dsq.vin == usedVin) {
                        fUsed = true;
                        break;
                    }
                ...
                CProsperitynode* pmn = mnodeman.Find(dsq.vin);
                if (pmn == NULL) {
                    LogPrintf("DoAutomaticDenominating --- dsq vin %s is not in prosperitynode list!", dsq.vin.ToString());
                    continue;
                }

                LogPrintf("DoAutomaticDenominating --- attempt to connect to prosperitynode from queue %s\n", pmn->addr.ToString());
                lastTimeChanged = GetTimeMillis();

                // connect to Prosperitynode and submit the queue request
                CNode* pnode = ConnectNode((CAddress)addr, NULL, true);
                if (pnode != NULL) {
                    pSubmittedToProsperitynode = pmn;
                    vecProsperitynodesUsed.push_back(dsq.vin);
                    ...
                    strAutoDenomResult = _("Error connecting to Prosperitynode.");
                    dsq.time = 0; //remove node
                    continue;
                }
            }
        ...
        while (i < 10) {
            CProsperitynode* pmn = mnodeman.FindRandomNotInVec(vecProsperitynodesUsed, ActiveProtocol());
            if (pmn == NULL) {
                LogPrintf("DoAutomaticDenominating --- Can't find random prosperitynode!\n");
                strAutoDenomResult = _("Can't find random Prosperitynode.");
                return false;
            }
            ...
            lastTimeChanged = GetTimeMillis();
            LogPrintf("DoAutomaticDenominating --- attempt %d connection to Prosperitynode %s\n", i, pmn->addr.ToString());
            CNode* pnode = ConnectNode((CAddress)pmn->addr, NULL, true);
            if (pnode != NULL) {
                pSubmittedToProsperitynode = pmn;
                vecProsperitynodesUsed.push_back(pmn->vin);
                ...
                vecProsperitynodesUsed.push_back(pmn->vin); // postpone MN we wasn't able to connect to
                i++;
                continue;
            }
        }

        strAutoDenomResult = _("No compatible Prosperitynode found.");
        return false;
    }
...
std::string CObfuscationPool::GetMessageByID(int messageID)
{
    ...
    case ERR_MN_LIST:
        return _("Not in the Prosperitynode list.");
    case ERR_MODE:
        return _("Incompatible mode.");
    case ERR_NON_STANDARD_PUBKEY:
        return _("Non-standard public key detected.");
    case ERR_NOT_A_MN:
        return _("This is not a Prosperitynode.");
    case ERR_QUEUE_FULL:
        return _("Prosperitynode queue is full.");
    case ERR_RECENT:
...
bool CObfuscationQueue::Sign()
{
    ...
    if (!obfuScationSigner.SetKey(strFundamentalNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CObfuscationQueue():Relay - ERROR: Invalid Prosperitynodeprivkey: '%s'\n", errorMessage);
        return false;
    }
...
bool CObfuscationQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        if (!obfuScationSigner.VerifyMessage(pmn->pubKeyProsperitynode, vchSig, strMessage, errorMessage)) {
            return error("CObfuscationQueue::CheckSignature() - Got bad Prosperitynode address signature %s \n", vin.ToString().c_str());
        }
...
void CObfuscationPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    ...
    CNode* pnode = FindNode(pSubmittedToProsperitynode->addr);
    if (pnode != NULL) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("Divitae-obfuscation");
    ...
    while (true) {
        ...
        if (prosperitynodeSync.IsBlockchainSynced()) {
            ...
            if (c % PROSPERITYNODE_PING_SECONDS == 1) activeProsperitynode.ManageStatus();

            if (c % 60 == 0) {
                mnodeman.CheckAndRemove();
                mnodeman.ProcessProsperitynodeConnections();
                prosperitynodePayments.CleanPaymentList();
                CleanTransactionLocksList();
            }

            //if(c % PROSPERITYNODES_DUMP_SECONDS == 0) DumpProsperitynodes();
