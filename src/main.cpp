...
// Copyright (c) 2018-2019 The DIVIT developers
...
#if defined(NDEBUG)
#error "DIVIT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uDIVIT) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zDIVIT is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zDIVIT spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zDIVIT without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zDIVIT spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zDIVIT spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zDIVIT spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zDivitae");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zDIVIT mints are not already known
        ...
        if (nFees > (PROSPERITYNODE_AMOUNT - FN_MAGIC_AMOUNT - 0.1*COIN)){

            nFees = nFees - PROSPERITYNODE_AMOUNT + FN_MAGIC_AMOUNT;
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
...

    {
        ...
        if (nFees > PROSPERITYNODE_AMOUNT - 1*COIN)
            nFees = nFees - PROSPERITYNODE_AMOUNT;
...
//        return false;
...

    {
        ...
        if (nFees < PROSPERITYNODE_AMOUNT - FN_MAGIC_AMOUNT)
            return false;
    }
...
int64_t GetProsperitynodePayment(int nHeight, int64_t blockValue, int nProsperitynodeCount)
{
    int64_t ret = 0;
...
CAmount GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZDIVITStake)
{
    int64_t ret = 0;
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase() && !tx.IsZerocoinSpend()) {
        ...
        if (!tx.IsCoinStake()) {
            ...
            if (nTxFee > PROSPERITYNODE_AMOUNT)
                nTxFee = nTxFee - PROSPERITYNODE_AMOUNT;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from DIVIT
...
void RecalculateZVITSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zDIVIT supply
...
{
    // DIVIT: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZDIVITSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (!tx.IsCoinStake()){
                ...
                if (nFees > PROSPERITYNODE_AMOUNT)
                    nFees = nFees - PROSPERITYNODE_AMOUNT;
    ...
    //Track zDIVIT money supply in the block index
    if (!UpdateZDIVITSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zDIVIT supply for block=%s height=%d", __func__,
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // prosperitynode payments / budgets
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block, nHeight)) {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                return state.DoS(0, error("CheckBlock() : Couldn't find prosperitynode/budget payment"),
                        REJECT_INVALID, "bad-cb-payee");
            }
        } else {
            if (fDebug)
                LogPrintf("CheckBlock(): Prosperitynode payment check skipped on sync - skipping IsBlockPayeeValid()\n");
        }
    }
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zDIVIT spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zDIVIT serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zDIVIT stake block is less than allowed start height", __func__);

    if (CZVitStake* zDIVIT = dynamic_cast<CZVitStake*>(stake)) {
        CBlockIndex* pindexFrom = zDIVIT->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zDIVIT stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zDIVIT stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zDIVIT->GetValue()));
        if (nChecksum200 != zDIVIT->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zDIVIT->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZPIV() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zDIVIT fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zDIVIT mints and %d zDIVIT spends\n", __func__, nMints, nSpends);
    ...
    if (!fLiteMode) {
        if (prosperitynodeSync.RequestedProsperitynodeAssets > PROSPERITYNODE_SYNC_LIST) {
            obfuScationPool.NewBlock();
            prosperitynodePayments.ProcessBlock(GetHeight() + 10);
            budget.NewBlock();
        }
    }
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_PROSPERITYNODE_WINNER:
        if (prosperitynodePayments.mapProsperitynodePayeeVotes.count(inv.hash)) {
            prosperitynodeSync.AddedProsperitynodeWinner(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_VOTE:
        if (budget.mapSeenProsperitynodeBudgetVotes.count(inv.hash)) {
            prosperitynodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_PROPOSAL:
        if (budget.mapSeenProsperitynodeBudgetProposals.count(inv.hash)) {
            prosperitynodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    ...
    case MSG_BUDGET_FINALIZED:
        ...
        return false;
    case MSG_PROSPERITYNODE_ANNOUNCE:
        if (mnodeman.mapSeenProsperitynodeBroadcast.count(inv.hash)) {
            prosperitynodeSync.AddedProsperitynodeList(inv.hash);
            return true;
        }
        return false;
    case MSG_PROSPERITYNODE_PING:
        return mnodeman.mapSeenProsperitynodePing.count(inv.hash);
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_PROSPERITYNODE_WINNER) {
                    if (prosperitynodePayments.mapProsperitynodePayeeVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << prosperitynodePayments.mapProsperitynodePayeeVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_VOTE) {
                    if (budget.mapSeenProsperitynodeBudgetVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenProsperitynodeBudgetVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_PROPOSAL) {
                    if (budget.mapSeenProsperitynodeBudgetProposals.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenProsperitynodeBudgetProposals[inv.hash];
                ...
                if (!pushed && inv.type == MSG_PROSPERITYNODE_ANNOUNCE) {
                    if (mnodeman.mapSeenProsperitynodeBroadcast.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenProsperitynodeBroadcast[inv.hash];
                        pfrom->PushMessage("fnb", ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_PROSPERITYNODE_PING) {
                    if (mnodeman.mapSeenProsperitynodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenProsperitynodePing[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // DIVIT: We use certain sporks during IBD, so check to see if they are
        ...
        //prosperitynode signed transaction
        ...
        if (strCommand == "tx") {
            ...
            //these allow prosperitynodes to publish a limited amount of free transactions
            vRecv >> tx >> vin >> vchSig >> sigTime;

            CProsperitynode* pmn = mnodeman.Find(vin);
            if (pmn != NULL) {
                if (!pmn->allowFreeTx) {
                    //multiple peers can send us a valid prosperitynode transaction
                    if (fDebug) LogPrintf("dstx: Prosperitynode sending too many transactions %s\n", tx.GetHash().ToString());
                    return true;
                }
                ...
                if (!obfuScationSigner.VerifyMessage(pmn->pubKeyProsperitynode, vchSig, strMessage, errorMessage)) {
                    LogPrintf("dstx: Got bad prosperitynode address signature %s \n", vin.ToString());
                    //pfrom->Misbehaving(20);
                    return false;
                }

                LogPrintf("dstx: Got Prosperitynode transaction %s\n", tx.GetHash().ToString());
    ...
    // getaddr message mitigates the attack.
        ...
        prosperitynodePayments.ProcessMessageProsperitynodePayments(pfrom, strCommand, vRecv);
        ProcessMessageSwiftTX(pfrom, strCommand, vRecv);
        ProcessSpork(pfrom, strCommand, vRecv);
        prosperitynodeSync.ProcessMessage(pfrom, strCommand, vRecv);
