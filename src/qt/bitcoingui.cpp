...
// Copyright (c) 2015-2017 The DIVIT developers
                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    QString windowTitle = tr("DIVIT Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a DIVIT address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and Divitae: URIs)"));
    ...
    privacyAction->setStatusTip(tr("Privacy Actions for zDIVIT"));
    ...
    QSettings settings;
    if (settings.value("fShowProsperitynodesTab").toBool()) {
        prosperitynodeAction = new QAction(QIcon(":/icons/prosperitynodes"), tr("&Prosperitynodes"), this);
        prosperitynodeAction->setStatusTip(tr("Browse prosperitynodes"));
        prosperitynodeAction->setToolTip(prosperitynodeAction->statusTip());
        prosperitynodeAction->setCheckable(true);
#ifdef Q_OS_MAC
        prosperitynodeAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_6));
#else
        prosperitynodeAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
#endif
        tabGroup->addAction(prosperitynodeAction);
        connect(prosperitynodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
        connect(prosperitynodeAction, SIGNAL(triggered()), this, SLOT(gotoProsperitynodePage()));
    }
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About DIVIT Core"), this);
    aboutAction->setStatusTip(tr("Show information about DIVIT Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for DIVIT"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your DIVIT addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified DIVIT addresses"));
	...
	openFNConfEditorAction = new QAction(QIcon(":/icons/edit"), tr("Open &Prosperitynode Configuration File"), this);
    openFNConfEditorAction->setStatusTip(tr("Open Prosperitynode configuration file"));
    ...
    openAction->setStatusTip(tr("Open a DIVIT: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the DIVIT Core help message to get a list with possible DIVIT command-line options"));
...
//        toolbar->widgetForAction(spacer)->setObjectName("ToolbarSpacer");
        ...
        QSettings settings;
        if (settings.value("fShowProsperitynodesTab").toBool()) {
            toolbar->addAction(prosperitynodeAction);
        }
        if (settings.value("fShowMasternodesTab").toBool()) {
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    QSettings settings;
    if (settings.value("fShowProsperitynodesTab").toBool()) {
        prosperitynodeAction->setEnabled(enabled);
    }
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("DIVIT Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::gotoProsperitynodePage()
{
    QSettings settings;
    if (settings.value("fShowProsperitynodesTab").toBool()) {
        prosperitynodeAction->setChecked(true);
        if (walletFrame) walletFrame->gotoProsperitynodePage();
    }
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to DIVIT network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count)
{
    ...
    if (prosperitynodeSync.IsBlockchainSynced()) {
        ...
        if (prosperitynodeSync.IsSynced()) {
            ...
            nAttempt = prosperitynodeSync.RequestedProsperitynodeAttempt < PROSPERITYNODE_SYNC_THRESHOLD ?
                           prosperitynodeSync.RequestedProsperitynodeAttempt + 1 :
                           PROSPERITYNODE_SYNC_THRESHOLD;
            progress = nAttempt + (prosperitynodeSync.RequestedProsperitynodeAssets - 1) * PROSPERITYNODE_SYNC_THRESHOLD;
            progressBar->setMaximum(4 * PROSPERITYNODE_SYNC_THRESHOLD);
            progressBar->setFormat(tr("Synchronizing additional data: %p%"));
            progressBar->setValue(progress);
        }

        strSyncStatus = QString(prosperitynodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("DIVIT Core"); // default title
