...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DIVIT);
    ...
    if (!settings.contains("nAnonymizeDivitaeAmount"))
        settings.setValue("nAnonymizeDivitaeAmount", 1000);

    nAnonymizeDivitaeAmount = settings.value("nAnonymizeDivitaeAmount").toLongLong();

    if (!settings.contains("fShowProsperitynodesTab"))
        settings.setValue("fShowProsperitynodesTab", prosperitynodeConfig.getCount());
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDivitaeAmount"))
        SoftSetArg("-anonymizeDivitaeamount", settings.value("nAnonymizeDivitaeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in Divitae.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case ShowProsperitynodesTab:
            return settings.value("fShowProsperitynodesTab");
       ...
       case ZeromintPrefDenom:
        ...
        case AnonymizeDivitaeAmount:
            return QVariant(nAnonymizeDivitaeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case SpendZeroConfChange:
            ...
            break;
        case ShowProsperitynodesTab:
            if (settings.value("fShowProsperitynodesTab") != value) {
                settings.setValue("fShowProsperitynodesTab", value);
                setRestartRequired(true);
            }
            break;
	    case ShowMasternodesTab:
       ...
       case ZeromintPrefDenom:
        ...
        case AnonymizeDivitaeAmount:
            nAnonymizeDivitaeAmount = value.toInt();
            settings.setValue("nAnonymizeDivitaeAmount", nAnonymizeDivitaeAmount);
            emit anonymizeDivitaeAmountChanged(nAnonymizeDivitaeAmount);
            break;
        case CoinControlFeatures:
